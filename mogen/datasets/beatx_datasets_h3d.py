...
class BEATXDataseth3d(data.Dataset):
    def __init__(
        self,
        split,
        build_cache= False,
        ...
        **kwargs,
        ...
        super(BEATXDataseth3d, self).__init__()
        ...
        # breakpoint()
        preloaded_dir = "/Dataset4D/public/mas-liu.lianlian/output/RAGesture/data_h3d/" + loader_type + f"/{self.args.pose_rep}_cache" 
        print(f"加载数据的地址：{preloaded_dir}")



        # =========================================================
        # 🔥 修改点 1: 配置 H3D 路径与归一化文件
        # =========================================================
        self.h3d_dir = "/Dataset4D/public/mas-liu.lianlian/code/SynTalker/process_h3d_beatx/HumanML3D/new_joint_vecs"
        self.h3d_mean_path = "/Dataset4D/public/mas-liu.lianlian/code/SynTalker/process_h3d_amass/HumanML3D/Mean.npy"
        self.h3d_std_path = "/Dataset4D/public/mas-liu.lianlian/code/SynTalker/process_h3d_amass/HumanML3D/Std.npy"

        self.h3d_dim = 623 # 根据之前的讨论，保留全关节是 623 维
        
        if os.path.exists(self.h3d_mean_path):
            self.h3d_mean = np.load(self.h3d_mean_path)
            self.h3d_std = np.load(self.h3d_std_path)
            self.logger.info(f"✅ Loaded H3D Mean/Std from {self.h3d_mean_path}")
        else:
            self.logger.warning(f"❌ H3D Mean/Std not found at {self.h3d_mean_path}. Normalization will be skipped or incorrect!")
            self.h3d_mean = None
            self.h3d_std = None
        # =========================================================
    ...
    def cache_generation(self, out_lmdb_dir, disable_filtering, clean_first_seconds,  clean_final_seconds, is_test=False):
        ...
        for index, file_name in self.selected_file.iterrows():
            ...
            if "smplx" in self.args.pose_rep:
                ...
                # =========================================================
                # 🔥 修改点 2: 读取 H3D 数据并对齐 (插入在 Pose 读取之后)
                # =========================================================
                h3d_path = os.path.join(self.h3d_dir, f_name + ".npy")
                if os.path.exists(h3d_path):
                    h3d_raw = np.load(h3d_path) # (T_raw, 623)
                    
                    # 1. 降采样 (确保与 Pose 的 stride 一致)
                    h3d_each_file = h3d_raw[::stride]
                    # 2. 归一化 (Data - Mean) / Std
                    if self.h3d_mean is not None:
                        h3d_each_file = (h3d_each_file - self.h3d_mean) / self.h3d_std
                    else:
                        self.logger.warning(colored(f"Missing H3D file: {h3d_path}", "red"))

                # 3. 长度强制对齐 (取 Pose 和 H3D 的最小长度)
                min_len = pose_each_file.shape[0]
                
                # pose_each_file = pose_each_file[:min_len]
                # trans_each_file = trans_each_file[:min_len]
                # # exps = exps[:min_len]
                # shape_each_file = shape_each_file[:min_len]
                h3d_each_file = h3d_each_file[:min_len] # <--- 对齐 H3D
                ...
                if self.args.facial_rep is not None:
                    ...
                    # 🔥 修改：确保 Facial 也遵循 min_len 对齐
                    facial_each_file = pose_data["expressions"][::stride][:min_len]
            ...
            filtered_result = self._sample_from_clip(
                ...
                disable_filtering, clean_first_seconds, clean_final_seconds, is_test,h3d_each_file, # <--- 传入这个新参数
                ) 
    ...
    def _sample_from_clip(
        ...
        for i in range(num_subdivision): # cut into around 2s chip, (self npose)
            ...
            abs_start_time = start_idx / self.args.pose_fps
            sample_abs_start_list.append(abs_start_time)
            ...
            # 🔥 H3D 切片 (逻辑同 Pose)
            sample_h3d = h3d_each_file[start_idx:fin_idx]
            ...
            if len(sample_pose) != 0:
                ...
                # 🔥 Append H3D
                sample_h3d_list.append(sample_h3d)
        
        if len(sample_pose_list) > 0:
            with dst_lmdb_env.begin(write=True) as txn:
                for smp_idx, (pose, upper, face, lower, hands, audio, audenc, facial, shape, word, word_enc, text_f, disco, textsegs, vid, emo, sem, semscore, trans, prom,h3d_motion,abs_start) in enumerate(zip(
                    sample_pose_list,
                    sample_pose_upper_list,
                    sample_pose_face_list,
                    sample_pose_lower_list,
                    ...
                    v = [pose, upper, face, lower, hands, audio, audenc, facial, shape, word, word_enc, text_f, disco, textsegs, emo, sem, semscore, vid, trans, prom, h3d_motion, abs_start, f_name + "/" + str(smp_idx)]
    ...
    def __getitem__(self, idx):
        ...
        with self.lmdb_env.begin(write=False) as txn:
            ...
            if len(sample) == 20:
                ...
                tar_pose, tar_upper, tar_face, tar_lower, tar_hands, in_audio, in_audenc, in_facial, in_shape, in_word, in_word_enc, in_textf, disco, textsegs, emo, sem, semscore, vid, trans, prom, motion_h3d, abs_start, f_name = sample
            ...
            # 🔥 H3D 转 Tensor
            motion_h3d = torch.from_numpy(motion_h3d).float()
            ...
            return {
                ...
                "motion_h3d": motion_h3d, # <--- 加入返回字典
                "abs_start_time": float(abs_start),
