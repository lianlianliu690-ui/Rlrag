...
class RetrievalDatabase(nn.Module):
    ...
    def __init__(
        ...
        if new_lmdb_cache:
            ...
            for smp_idx, smp in tqdm(enumerate(dataset)):
                ...
                if new_lmdb_cache or not os.path.exists(os.path.join(lmdb_paths, "idx_2_sense")):
                    self.idx_2_sense[smp["sample_name"]] = [speaker_id] + [(d[1], d[0]) for d in smp["discourse"]]

                if new_lmdb_cache or not os.path.exists(os.path.join(lmdb_paths, "idx_2_discbounds")):
                    self.idx_2_discbounds[smp["sample_name"]] = [(d[1], d[0], d[4], d[5], d[6], d[7]) for d in smp["discourse"]]
        ...
        # [新增] 加载 NetworkX 知识图谱 (用于图遍历和 Path B 向量检索)
        self.kg_graph = None
        if kg_path and os.path.exists(kg_path):
            print(f"[KG] Loading Knowledge Graph from {kg_path}...")
            try:
                self.kg_graph = nx.read_gexf(kg_path)
                print(f"[KG] ✅ Loaded Graph with {self.kg_graph.number_of_nodes()} nodes.")
            except Exception as e:
                print(f"[KG] ❌ Error loading KG GEXF: {e}")

    def multi_scale_sliding_window(self, raw_motion, text_desc, fps=15):
        """
        [新增] 2.2 多尺度滑动窗口：在动作序列上寻找最佳时域片段
        这里使用简单的动能(Velocity Energy)作为启发式算法。
        """
        # 定义尺度 (秒)
        scales = [1.0, 2.0, 3.0] 
        
        if torch.is_tensor(raw_motion):
            motion_np = raw_motion.detach().cpu().numpy()
        else:
            motion_np = raw_motion
            
        T = motion_np.shape[0]
        if T == 0: return (0.0, 1.0)

        # 计算动能曲线 (Velocity Norm)
        if len(motion_np.shape) == 1: motion_np = motion_np[:, None]
        velocity = np.diff(motion_np, axis=0) # [T-1, D]
        energy = np.linalg.norm(velocity, axis=1) # [T-1]
        
        best_score = -1.0
        best_window = (0.0, float(T) / fps) # 默认全长

        for duration in scales:
            win_len = int(duration * fps)
            if win_len >= len(energy): continue
            
            stride = int(0.2 * fps)
            for i in range(0, len(energy) - win_len, stride):
                current_energy = np.sum(energy[i : i+win_len])
                if current_energy > best_score:
                    best_score = current_energy
                    best_window = (float(i) / fps, float(i + win_len) / fps)
    ...
    def retrieve(
        ...
        idx=None,
        device='cpu',    # [新增] 传入设备
        text_model=None, # [新增] 传入文本编码器 (用于 Path B 向量检索)
    ):
        assert retr_method in ["gesture_type", "discourse", "llm"] 
        ...
        # Testing logic (Cache Hit)
            ...
            if len(db_indexes) == 0:
                print(f"WARNUNG: No samples found for idx {idx} for retr_method {retr_method}")
        ...
        # Testing logic (Cache Miss - Perform Retrieval)
        else:
            method_args = {}
            for retr_m in self.retrieval_method:
                ...
                if retr_m == "gesture_type":
                    ...
                    method_args[retr_m]["db_idx_2_gesture_labels"] = self.idx_2_gesture_labels
                    ...
                    # [关键新增] 传入图谱、设备和模型以支持混合检索 Path B
                    method_args[retr_m]["kg_graph"] = getattr(self, 'kg_graph', None) 
                    method_args[retr_m]["device"] = device
                    method_args[retr_m]["text_model"] = text_model
            ...
            if not self.training:
                # 调用指定的检索方法函数
                sample_indexes, sample_bounds, query_bounds = self.retrieval_method[retr_method](
                    **method_args[retr_method]
                )

                # ================= [新增] Path B: 向量检索后处理 (Alignment) =================
                # 对 'vector_search' 结果执行滑动窗口对齐
                if retr_method == "llm":
                    for q_idx, bounds_dict in sample_bounds.items():
                        # 使用 list(bounds_dict.items()) 避免迭代时修改字典报错
                        for smp_idx, bound_info in list(bounds_dict.items()):
                            gest_desc = bound_info[0]
                            gest_type = bound_info[1]
                            
                            # 拦截 vector_search 结果
                            if gest_type == "vector_search":
                                try:
                                    # smp_idx 这里应该是 dataset 中的整数索引 (由 llm_retrieval 里的 Path B 处理逻辑决定)
                                    # llm_retrieval 里我们之前是用字符串 id，
                                    # 注意：llm_retrieval 需要确保返回的是 int(dataset_index)
                                    # 假设 Path B 里已经把 smp_idx 转换为了 int
                                    smp_idx_int = int(smp_idx) 
                                    
                                    if smp_idx_int < len(self.dataset):
                                        raw_motion = self.dataset[smp_idx_int]["motion"]
                                        
                                        # 执行多尺度滑动窗口，计算精确的 start/end
                                        new_start, new_end = self.multi_scale_sliding_window(
                                            raw_motion, 
                                            gest_desc, 
                                            fps=self.motion_fps
                                        )
                                    ...
                                    # print(f"[Warn] Alignment failed for sample {smp_idx}: {e}")
                                    continue
    ...
    def forward(
        ...
        # 获取 Text Encoder (用于 Path B 向量检索)
        # 假设 gesture_rep_encoder 或者 self 本身有 text_encoder
        # 这里需要您根据实际模型架构传入，这里假设传入的 gesture_rep_encoder 有 text_encoder
        text_encoder_func = None
        if hasattr(gesture_rep_encoder, 'text_encoder'):
            text_encoder_func = gesture_rep_encoder.text_encoder
        ...
        for b_ix in range(B):
            retr_indexes, retr_bounds, query_bounds = self.retrieve(
                ...
                device=device,                # [新增]
                text_model=text_encoder_func, # [新增]
            )
            ...
            for query_point_idx, smp_idxs in retr_indexes.items():
                ...
                for smp_idx in smp_idxs:
                    ...
                    # ================= [新增: LLM模式下的 KG 调制] =================
                    # 确保 self 中有 kg_map (需要在 __init__ 中加载)
                    if retrieval_method == "llm" and hasattr(self, 'kg_map'):
                        # 1. 拿到 LLM 原始生成的词
                        if query_point_idx in query_bounds:
                            llm_gen_word = query_bounds[query_point_idx][0].lower()
                            
                            # 2. 查图谱，获取目标词和强度
                            if llm_gen_word in self.kg_map:
                                node = self.kg_map[llm_gen_word][0]
                                target_label = node.get('target_label', llm_gen_word)
                                intensity = node.get('intensity', 0.5)
                                
                                # 3. 检查检索回来的动作到底是哪个词?
                                if query_point_idx in retr_bounds and smp_idx in retr_bounds[query_point_idx]:
                                    retrieved_word = retr_bounds[query_point_idx][smp_idx][0].lower()
                                    
                                    # 核心判断：检索词 == KG目标词 (发生替换) 且 != LLM原词
                                    if retrieved_word == target_label and retrieved_word != llm_gen_word:
                                        scale = 1.0 + (intensity - 0.5) * 1.0
                                        scale = max(0.5, min(scale, 1.5))
                                        # print(f"[KG-Mod] {llm_gen_word} -> {retrieved_word} | Scale: {scale:.2f}")

                    # 应用缩放 (修改送入 Encoder 的动作)
                    if scale != 1.0:
                        retr_motion = retr_motion * scale
                        retr_motion_upper = retr_motion_upper * scale
                        retr_motion_hands = retr_motion_hands * scale
                        # 如果需要，也可以缩放其他部位，如:
                        # retr_motion_lower = retr_motion_lower * scale
...
class ReGestureTransformer(DiffusionTransformer):
    ...
    # ========用于预计算和准备条件输入，为扩散模型的生成过程提供必要的条件信息===========
    def get_precompute_condition(
        ...
        **kwargs,
        ...
        if re_dict is None and self.database is not None:
            ...
            re_dict = self.database(    # RetrievalDatabase 调用数据库进行检索,传入检索条件、运动长度、设备信息等
                retr_conditions,
                motion_length,
                device,
